...
TEST(Input, CorrentInputForEvents) {
  ...
  auto id = ds.GetServer().CreateWindow({}).value();

  ds.GetServer().InjectKeyEvent(id, eInputKey::Apostrophe, true);
  EXPECT_TRUE(InputState::IsPressed(eInputKey::Apostrophe));
  ds.GetServer().InjectKeyEvent(id, eInputKey::F10, false);
  EXPECT_FALSE(InputState::IsPressed(eInputKey::F10));

  ds.GetServer().InjectMouseEvent(id, {.position = vec2<float>(12.0f, 14.0f)});
  ...
  ds.GetServer().InjectMouseEvent(id, {.scroll_delta = vec2<int16_t>{-5, 2}});
...
TEST(Input, CorrectMousePositionDelta) {
  ...
  ds.GetServer().InjectMouseEvent(id, {.position = new_position});
